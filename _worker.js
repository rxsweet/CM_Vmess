...
export default {
	async fetch(request, env) {
		...
		if (!userAgent.includes('subconverter') && MamaJustKilledAMan.some(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead => userAgentHeader.toLowerCase().includes(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead)) && MamaJustKilledAMan.length > 0) {
			...
			subconverterUrl = `https://${subconverter}/sub?target=clash&url=${encodeURIComponent(subconverterUrl)}&config=${encodeURIComponent(subconfig)}&&emoji=false&insert=false&scv=true&udp=true&list=false&tfo=false&fdn=false&sort=false`;
